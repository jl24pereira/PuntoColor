...
public class CategoriaProducto {
    ...
    public CategoriaProducto(int id, String nombre, String detalle){
        this.id = id;
        this.nombre = nombre;
    ...
    public String generarCodigo(){
        ...
        int count = create.select(count())
                .from(PRODUCTO)
                .where(PRODUCTO.CODIGO.like("%"+codigo+"%")).fetchOne(0, int.class);
        if(count == 0){
            codigo = codigo + "0001";
        }else{
            String ultimo = create.select(PRODUCTO.CODIGO)
                    .from(PRODUCTO)
